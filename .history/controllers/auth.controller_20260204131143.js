...
const securityCookie = process.env.PROD === "true";
let sameSiteCookie = process.env.PROD === "true" ? "none" : "lax";
...
export const confirm = async (req, res) => {
  ...
  try {
    ...
    if (error || !data?.[0]?.success) return res.status(400).send("Invalid or expired token");
...
export const login = async (req, res) => {
  ...
  try {
    ...
    res.cookie("access_token", accessToken, { httpOnly: true, secure: securityCookie, sameSite: sameSiteCookie, maxAge: 15*60*1000 });
    res.cookie("refresh_token", refreshToken, { httpOnly: true, secure: securityCookie, sameSite: sameSiteCookie, maxAge: 7*24*60*60*1000 });
...
export const refresh = (req, res) => {
  ...
  try {
    ...
    res.cookie("access_token", newAccess, { httpOnly: true, secure: securityCookie, sameSite: sameSiteCookie, maxAge: 10*60*1000 });
